...
class ProductionHandler(QuietHandler):
    ...
    def do_GET(self):
        ...
        # 1. Web Dashboard (multi-page portal)
        PAGES = {
            "/":           "dashboard.html",
        ...
        if parsed.path in PAGES:
            tpl = os.path.join(os.path.dirname(__file__), "templates", PAGES[parsed.path])
            try:
        ...
        if parsed.path.startswith("/static/") or parsed.path.startswith("/assets/"):
            ...
            if os.path.commonpath([base_dir, safe_target]) == base_dir and os.path.isfile(safe_target):
                ...
                elif safe_target.endswith(".jpg") or safe_target.endswith(".jpeg"): content_type = "image/jpeg"
                elif safe_target.endswith(".ttf"): content_type = "font/ttf"
                elif safe_target.endswith(".js"): content_type = "text/javascript"
                elif safe_target.endswith(".css"): content_type = "text/css"
                elif safe_target.endswith(".svg"): content_type = "image/svg+xml"
                elif safe_target.endswith(".webp"): content_type = "image/webp"
                elif safe_target.endswith(".gif"): content_type = "image/gif"
                elif safe_target.endswith(".webm"): content_type = "video/webm"
                elif safe_target.endswith(".mp4"): content_type = "video/mp4"
                elif safe_target.endswith(".ico"): content_type = "image/x-icon"
                elif safe_target.endswith(".html"): content_type = "text/html; charset=utf-8"
